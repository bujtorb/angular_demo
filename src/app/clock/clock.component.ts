...
export class ClockComponent implements OnInit {
  ...
  setDate() {
    ...
    this.renderer.setStyle(this.secondHand.nativeElement, 'transform',  `rotate(${secDeg}deg)`);
    const min = now.getMinutes();
    const minDeg = ((min / 60) * 360) + 90;
    this.renderer.setStyle(this.minutesHand.nativeElement, 'transform',  `rotate(${minDeg}deg)`);

    const hour = now.getHours();
    const hourDeg = ((hour / 60) * 360) + 90;
    this.renderer.setStyle(this.hourHand.nativeElement, 'transform',  `rotate(${hourDeg}deg)`);


    if (secDeg === 90 || minDeg === 90 || hourDeg === 90 ) {

        this.renderer.setStyle(this.secondHand.nativeElement, 'transition', 'none');
        this.renderer.setStyle(this.minutesHand.nativeElement, 'transition', 'none');
